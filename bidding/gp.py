...
def evolve(pc,popsize,rankfunction,maxgen=500,treedepth=5,
  ...
  population=[makerandomtree(pc, treedepth) for i in range(popsize)]
...
def bidgame(player1, player2):
  ...
  while has_winner(board)==0 and moves<100:
    ...
    player1_bid = player1.evaluate(create_player_board(board, 1, money['player1'], money['player2']))
    player2_bid = player2.evaluate(create_player_board(board, 2, money['player2'], money['player1']))
...
def create_player_board(board, i, money, opp_money):
  if i==2:
    board.reverse()
  player_board = board
  player_board.append(board.index(1))
  player_board.append(money)
  player_board.append(opp_money)
  
  return player_board

def tournament(pl, playgame=bidgame):
  # Count losses
